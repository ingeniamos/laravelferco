...
$(document).ready(function ()
{
	...
	$("#produccion_modificar_ord_produccion").bind('change', function (event)
	{
		...
		{
			var item_value = $("#produccion_modificar_ord_produccion").jqxComboBox('getSelectedItem');
			if (item_value)
			{
			...
			{
				var value = $("#produccion_modificar_ord_produccion").val();
				
				var item = $("#produccion_modificar_ord_produccion").jqxComboBox('getItems');
				for (var i = 0; i < item.length; i++)
				{
					if (item[i].label == value)
					{
						$("#produccion_modificar_ord_produccion").jqxComboBox('selectItem', item[i].value);
						return;
					}
				}
				ClearDocument();
	...
	$("#produccion_modificar_ord_produccion").bind('bindingComplete', function (event) {
		if (Ord_Produccion != "")
			$("#produccion_modificar_ord_produccion").jqxComboBox('selectItem', Ord_Produccion);
	});
	OrdenSource.data = {"Produccion_Ordenes": $("#produccion_modificar_estado").val()};
	OrdenDataAdapter = new $.jqx.dataAdapter(OrdenSource);
	$("#produccion_modificar_ord_produccion").jqxComboBox({source: OrdenDataAdapter});
	...
	$("#produccion_modificar_codigo2").bind('select', function (event) {
		if (event.args) {
			CodFabID = event.args.item.value;
			clearTimeout(Timer);
			Timer = setTimeout(function(){
	...
	$("#produccion_modificar_codigo2").bind('close', function () {
		if (CodFabID != "" && CodFabID != -1) {
			$("#produccion_modificar_producto2").val(CodFabID);
			//Load
		}
	...
	$("#produccion_modificar_producto2").bind('select', function (event) {
		if (event.args) {
			CodFabID = event.args.item.value;
			clearTimeout(Timer);
			Timer = setTimeout(function(){
	...
	$("#produccion_modificar_producto2").bind('close', function () {
		if (CodFabID != "" && CodFabID != -1) {
			$("#produccion_modificar_codigo2").val(CodFabID);
			//Load
		}
