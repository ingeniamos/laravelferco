...
$(document).ready(function ()
{
		...
		var LoadTimer = setTimeout(function()
		{
			...
			var GetValuesAdapter = new $.jqx.dataAdapter(GetValuesSource,{
				...
				loadComplete: function ()
				{
					...
					$("#maquinaria_modificar_clasificacion").val(records[0]["Clasificacion"]);
					$("#maquinaria_modificar_tipo").val(records[0]["Tipo"]);
					$("#maquinaria_modificar_motivo").val(records[0]["Motivo"]);
					$("#maquinaria_modificar_diagnostico").val(records[0]["Diagnostico"]);
	...
	$("#maquinaria_modificar_clasificacion").bind('change', function (event) {
		if (!event.args)
		{
	...
	$("#maquinaria_modificar_clasificacion").bind('select', function (event) {
		if (event.args) {
			clearTimeout(Timer1);
	...
	function Guardar_Cambios()
	{
		...
		if ($("#maquinaria_modificar_clasificacion").val() <= 0 | $("#maquinaria_modificar_clasificacion").val() == "") {
		...
		if ($("#maquinaria_modificar_tipo").val() <= 0 | $("#maquinaria_modificar_tipo").val() == "") {
		...
		if ($("#maquinaria_modificar_motivo").val() <= 0 | $("#maquinaria_modificar_motivo").val() == "") {
		...
		if ($("#maquinaria_modificar_diagnostico").val() <= 0 | $("#maquinaria_modificar_diagnostico").val() == "") {
		...
		myarray["Clasificacion"] = $("#maquinaria_modificar_clasificacion").val();
		myarray["Tipo"] = $("#maquinaria_modificar_tipo").val();
		myarray["Motivo"] = $("#maquinaria_modificar_motivo").val();
		myarray["Diagnostico"] = $("#maquinaria_modificar_diagnostico").val();
	...
	$("#maquinaria_modificar_guardar").bind('click', function ()
	{
		Guardar_Cambios();
	});
	...
	$("#maquinaria_modificar_nuevo").bind('click', function ()
	{
		ClearDocument();
	});
	...
	$("#maquinaria_modificar_imprimir").bind('click', function ()
	{
		//
	});
	...
	<?php
		if (isset($_SESSION["UserID"]) && $_SESSION["UserID"] != "")
		{
			...
			if ($data[0]["Lvl"] != "Administrador") {
				for ($i = 0; $i < $num; $i++)
				{
					if ($data[$i]["Modulo"] == "Maquinaria" && $data[$i]["SubModulo"] == "Modificar" && $data[$i]["Guardar"] == "false")
					{
			?>
			...
			<?php
					...
					if ($data[$i]["Modulo"] == "Ventas" && $data[$i]["SubModulo"] == "Modificar" && $data[$i]["Imprimir"] == "false")
					{
			?>
	...
	<table cellpadding="1" cellspacing="0" style="margin-bottom:15px;">
		...
		<tr>
			<td rowspan="3" style="padding:0px;">
				<textarea rows="5" cols="51" id="maquinaria_modificar_causa" maxlength="200" style="resize:none;"></textarea>
			</td>
			<td rowspan="3" style="padding:0px;">
				<textarea rows="5" cols="51" id="maquinaria_modificar_procedimiento" maxlength="200" style="resize:none;"></textarea>
			</td>
