	...
	$.ajax({
		...
		success: function (data, status, xhr)
		{
			...
			else if ("<?php echo $OrderBy; ?>" == "Cliente")
			{
				...
				for (var i = 0; i < len; i++)
				{
					if (Viejo_ID != data[i]["ClienteID"] && data[i]["ClienteID"] != "")
					{
						...
						table += "			<input type=\"text\" style=\"width: 250px; height: 20px; text-align:left;\" class=\"myinput\" readonly value=\""+data[i]["Nombre"]+"\"/>";
						table += "		</td>";
						table += "		<td style=\"width:9%;\">";
						table += "			Total Peso";
						table += "		</td>";
			...
			else if ("<?php echo $OrderBy; ?>" == "Vehiculo")
			{
				for (var i = 0; i < len; i++)
				{
					if (Viejo_ID != data[i]["Placa"] && data[i]["Placa"] != undefined && data[i]["ClienteID"] != "")
					{
						var array = {};
						array["Peso"] = TmpPeso;
						array["Valor"] = TmpValor;
						...
						if (i == 0)
							myarray[""+data[i]["Placa"]+""] = array;
						else
							myarray[""+data[i-1]["Placa"]+""] = array;
						
						Viejo_ID = data[i]["Placa"];
						TmpPeso = data[i]["Peso"];
						TmpValor = data[i]["Valor"];
					}
					else if (Viejo_ID == data[i]["Placa"] && data[i]["Placa"] != undefined && data[i]["ClienteID"] != "")
					{
						TmpPeso = + TmpPeso + parseFloat(data[i]["Peso"]);
						TmpValor = + TmpValor + parseFloat(data[i]["Valor"]);
					}
				...
				for (var i = 0; i < len; i++)
				{
					if (Viejo_ID != data[i]["Placa"])
					{
						Viejo_ID = data[i]["Placa"];
						if (i != 0)
							table += "</table><br />";
						
						var Peso = parseFloat(myarray[""+data[i]["Placa"]+""]["Peso"]);
						var Valor = parseFloat(myarray[""+data[i]["Placa"]+""]["Valor"]);
						...
						table += "		</td>";
						table += "		<td style=\"width:18%;\">";
						table += "			<input type=\"text\" style=\"width: 120px; height: 20px; text-align:center;\" class=\"myinput\" readonly value=\""+data[i]["Placa"]+"\"/>";
						table += "		</td>";
						table += "		<td style=\"width:9%;\">";
						table += "			Conductor";
						table += "		</td>";
						table += "		<td style=\"width:36%;\">";
						table += "			<input type=\"text\" style=\"width: 250px; height: 20px; text-align:left;\" class=\"myinput\" readonly value=\""+data[i]["Conductor"]+"\"/>";
						table += "		</td>";
						table += "		<td style=\"width:9%;\">";
						table += "			Total Peso";
						table += "		</td>";
