...
else if (isset($_GET['Caja_cheques']))
{
	...
	$output .= "Valor\t";
	$output .= "Fecha cheque\t";
	$output .= "Banco destino\t";
	$output .= "# Cuenta\t";
	$output .= "Recibo de caja\t";
	$output .= "Fecha\t";
	$output .= "DigitadorID\t";
	$output .= "\n";
	// End Header
	...
	// $query = "SELECT * FROM cheques WHERE DATE(fecha) BETWEEN '".$Fecha_Ini."' AND '".$Fecha_Fin."' AND estado = 'Aprobado'";
	// $query .= (isset($_GET['ClienteID']) && $_GET['ClienteID'] != "") ? " AND cliente_id = '".$_GET['ClienteID']."'":"";
	...
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC))
		{
			$data[] = array (
				'Estado' => "".$row["estado"],
				'Titular' => isset($ClienteNombre[$row["cliente_id"]]) ? $ClienteNombre[$row["cliente_id"]]:"No Existe!",
				'ClienteID' => $row["cliente_id"],
				'#Cheque' => $row["cheque"],
				'Banco' => $row["banco"],
				'Valor' => $row["valor"],
				'Fecha cheque' => $row["fecha_cheque"]." 00:00:00",
				'Banco destino' => $row["banco_destino"],
				'# Cuenta' => $row["cuenta"],
				'Recibo de caja' => $row["caja_recibo"],
		...
		foreach ($data AS $row)
		{
			$schema_insert = "";
			//
			$schema_insert .= "".$row["Estado"]."\t";
			$schema_insert .= "".$row["Titular"]."\t";
			$schema_insert .= "".$row["ClienteID"]."\t";
			$schema_insert .= "".$row["#Cheque"]."\t";
			$schema_insert .= "".$row["Banco"]."\t";
			$schema_insert .= "".$row["Valor"]."\t";
			$schema_insert .= "".$row["Fecha cheque"]."\t";
			$schema_insert .= "".$row["Banco destino"]."\t";
			$schema_insert .= "".$row["# Cuenta"]."\t";
			$schema_insert .= "".$row["Recibo de caja"]."\t";
			$schema_insert .= "".$row["Fecha"]."\t";
...
else if (isset($_GET['Cartera_AplicarPago']))
{
	...
	$query = "SELECT * FROM mov_clientes WHERE";
	$query .= " estado = 'Aprobado' ";
	$query .= " AND saldo > 0 ";
	$query .= " AND tipo_movimiento = 'debito' ";
	//$query .= (isset($_GET['TipoMovimiento']) && $_GET['TipoMovimiento'] != "") ? " AND tipo_movimiento = '".$_GET['TipoMovimiento']."'":"";
	$query .= (isset($_GET['ClienteID']) && $_GET['ClienteID'] != "") ? " AND cliente_id = '".$_GET['ClienteID']."'":"";
	$query .= (isset($_GET['VendedorID']) && $_GET['VendedorID'] != "") ? " AND vendedor_codigo = '".$_GET['VendedorID']."'":"";
	$query .= (isset($_GET['CobradorID']) && $_GET['CobradorID'] != "") ? " AND cobrador_codigo = '".$_GET['CobradorID']."'":"";
	...
	$filename = "Cartera_Aplicar_" .$fechafile ."_" .$_GET['CobradorID'] .$extension ;	
...
else if (isset($_GET['Reportes_Ventas']))// Cambiar a buscar el peso en la misma fact_final
{
	...
	if (mysql_num_rows($result1) > 0)
	{
		while ($row = mysql_fetch_array($result1))
		{
			...
			$schema_insert .= ($row['peso'] != "") ? "".$row['peso']."\t":"NULL\t";
			$schema_insert .= ($row['placa'] != "") ? "".$row['placa']."\t":"NULL\t";
	...
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_array($result))
		{
			...
			$data[] = array(
				...
				"Peso" => isset($ProductoPeso[$row["codigo"]]) ? $ProductoPeso[$row["codigo"]]* $Cantidad:"",
				"Cantidad" => $Cantidad,
