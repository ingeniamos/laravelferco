...
function Anular_CxPMovs ($Entrada, $Interno, $Motivo)
{
	$query = "SELECT tipo_movimiento, valor, saldo, caja_interno, caja_recibo FROM cxp_movs 
	WHERE compra_interno = '".$Interno."' AND compra_entrada = '".$Entrada."'";
...
function Anular_ProduccionMovs ($ClienteID, $Ord_Compra, $Interno, $Motivo)
{
	...
	motivo_anulado = '".$Motivo."', 
	fecha_anulado = NOW() 
	WHERE interno = '".$Interno."' AND orden_compra = '".$Ord_Compra."' AND cliente_id = '".$ClienteID."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
	if (mysql_affected_rows() <= 0)
		return false;
	else
...
function Anular_Final ($ClienteID, $Ord_Compra, $Interno, $From, $Motivo)
{
	switch ($From)
	{
		...
		break;
		case "Compra":
			Anular_CxPMovs($Ord_Compra, $Interno, $Motivo);
			Del_InventoryMovs($Interno);
		break;
		case "Cartera":
...
if (isset($_GET['Terceros_Crear']))
{
	...
	foreach($data as $item)
	{
		...
		if ($final == false)
		{
			...
			'".$Cupo_Credito."', '".$Vigencia_Cupo_Credito."', '".$Cupo_Adicional."', '".$Vigencia_Cupo_Adicional."', 'false', 
...
else if (isset($_GET['Ventas_Crear']))
{
	//Parte 1
	$Duplicar = false;
	$ClienteID = 0;
...
else if (isset($_GET['Ventas_Modificar']))
{
	...
	$data = $_GET['Ventas_Modificar'];
	$delete = true;
	$query = "SELECT codigo FROM fact_movs WHERE orden_compra = '".$data[0]['Ord_Compra']."' ";
	...
	if ($numrow > 0){
		while ($rows = mysql_fetch_array($result))
		{
			...
			if ($delete == true)
			{
				$query2 = "DELETE FROM fact_movs WHERE codigo = '".$rows['codigo']."' AND orden_compra = '".$data[0]['Ord_Compra']."' ";
	...
	foreach($data as $item) {
		...
		if ($final == false) {
			...
			{
				$query = "SELECT orden_produccion FROM produccion_final WHERE orden_compra = '".$Ord_Compra."' 
				AND interno = '".$Interno."' AND cliente_id ='".$ClienteID."'";
				$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
				...
				if (mysql_num_rows($result) <= 0)
				{
					...
					$query = "INSERT INTO produccion_final 
					(orden_produccion, orden_compra, interno, destino, cliente_id, fecha, estado, digitado_por, fecha_digitado) VALUES 
					('".$Ord_Produccion."', '".$Ord_Compra."', '".$Interno."', 'Cliente', '".$ClienteID."', '".$Fecha."', 'Pendiente', '".$_SESSION["UserCode"]."', NOW())";
					$result = mysql_query($query) or die ("SQL Error 2: " . mysql_error());
				}
				else
				{
					// BORRAR MOVIMIENTOS
					$query = "DELETE FROM produccion_movs WHERE orden_compra = '".$Ord_Compra."' 
					AND interno = '".$Interno."' AND cliente_id ='".$ClienteID."'";
					$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
				}
			}
		...
		$query = "SELECT `codigo` FROM fact_movs WHERE codigo = '".$Codigo."' AND `orden_compra` = '".$Ord_Compra."'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #3: ".mysql_error():"");
		...
		if (mysql_num_rows($result)>0){
			...
			WHERE codigo = '".$Codigo."' AND `orden_compra` = '".$Ord_Compra."'";
			$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #4: ".mysql_error():"");
		...
		if ($final == false)
		{
			...
			WHERE `orden_compra` = '".$Ord_Compra."'";
			$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #6: ".mysql_error():"");
			
			// Buscar Movimientos Previamente Generados por Ventas -> CREAR
			$query = "SELECT * FROM mov_clientes WHERE interno = '".$Interno."' AND orden_compra = '".$Ord_Compra."' 
			AND estado = 'Aprobado'";
			$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #7: ".mysql_error():"");
			if (mysql_num_rows($result)>0)
			{
				...
				{// Actualizar los Movimientos
					...
					WHERE tipo_movimiento = 'Credito' AND interno = '".$Interno."' AND orden_compra = '".$Ord_Compra."'";
					$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #9: ".mysql_error():"");
				}
			}
			else
			{
				...
				{// Crear los Movimientos
					...
					'".$Remision."', '".$Factura."', NOW(), 'Aprobado', '".$_SESSION["UserCode"]."', NOW(), 
...
else if (isset($_GET['Ventas_Modificar_Anular']))
{
	...
	if ($Ord_Compra != "")
	{
		$query = "SELECT id FROM produccion_final WHERE orden_compra = '".$Ord_Compra."'";
		$result = mysql_query($query) or die("SQL Error: " . mysql_error());
		if (mysql_num_rows($result) > 0)
		{
			$query = "SELECT id FROM produccion_final WHERE orden_compra = '".$Ord_Compra."' AND estado = 'Pendiente'";
			$result = mysql_query($query) or die("SQL Error: " . mysql_error());
			if (mysql_num_rows($result) > 0)
			{
				$query = "UPDATE produccion_final SET estado = 'Anulado', anulado_por = '".$_SESSION["UserCode"]."', 
				motivo_anulado = '".$Motivo."', 
				fecha_anulado = NOW() WHERE orden_compra = '".$Ord_Compra."'";
				$result = mysql_query($query) or die("SQL Error: " . mysql_error());
				$Mess = "OK";
			}
			else
		...
		else
		{
			...
			`motivo_anulado` = '".$Motivo."', `fecha_anulado` = NOW() WHERE `orden_compra` = '".$Ord_Compra."'";
			$result = mysql_query($query) or die("SQL Error: " . mysql_error());
			$Mess = "OK";
		}
	}
...
else if (isset($_GET['Ventas_Modificar_Crear']))
{
	...
	$query = "UPDATE fact_final SET `estado` = 'Creado', `digitado_por` = '".$_SESSION["UserCode"]."', 
	`fecha_digitado` = NOW() WHERE `orden_compra` = '".$Ord_Compra."'";
	$result = mysql_query($query) or die("SQL Error: " . mysql_error());
}
...
else if (isset($_GET['Ventas_Autorizar']))// Arreglar la visualizacion de VendedorID en el Listado para poder guardar aca.
{
	$Estado = isset($_GET['Estado']) ? $_GET['Estado']:"Creado";
	$Remision = isset($_GET['Remision']) ? $_GET['Remision']:0;
	...
	$Ord_Compra = isset($_GET['Ord_Compra']) ? $_GET['Ord_Compra']:0;
	$Ord_Produccion = isset($_GET['Ord_Produccion']) ? $_GET['Ord_Produccion']:0;
	...
	$query .= ($Estado == "Autorizado") ? ", autorizado_por = '".$_SESSION["UserCode"]."', fecha_autorizado = NOW()":"";
...
else if (isset($_GET['Ventas_Despachar']))
{
	...
	if ($last_parcial == true)
	{
		foreach($data as $item)
		{
			...
			if ($Cantidad2 > 0) {
				//-- Update Fact Movs
				$query = "UPDATE fact_movs SET cantidad = '".$Cantidad2."', cantidad_despachada = '0.00' WHERE 
				orden_compra = '".$Ord_Compra."' AND interno = '".$Interno."' AND codigo = '".$Codigo."'";
				$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
			}
			else {
			...
			if ($final == false)
			{
				if ($FormaPago == "Efectivo")
				{
					...
					'".$Remision."', '".$Factura."', NOW(), 'Aprobado', '".$_SESSION["UserCode"]."', NOW(), '".$_SESSION["UserCode"]."', 
				...
				if ($FormaPago == "Credito") {
					$query = "INSERT INTO mov_clientes 
					(cliente_id, tipo_movimiento, valor, saldo, interno, orden_compra, remision, factura, fecha, estado, 
					digitado_por, fecha_digitado, aprobado_por, cobrador_codigo) VALUES 
					('".$ClienteID."', 'Debito', '".$Total."', '".$Total."', '".$Interno."', '".$Ord_Compra."', '".$Remision."', 
					'".$Factura."', NOW(), 'Aprobado', '".$_SESSION["UserCode"]."', NOW(), '".$_SESSION["UserCode"]."', '".$CobradorID."')";
					$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #8: ".mysql_error():"");
				}
				$final = true;
			}
		...
		foreach($data as $item)
		{
			...
			if ($final == false)
			{
				if ($FormaPago == "Efectivo")
				{
					...
					(cliente_id, tipo_movimiento, valor, saldo, interno, orden_compra, remision, factura, fecha, estado, 
					digitado_por, fecha_digitado, aprobado_por, vendedor_codigo, cobrador_codigo, caja_interno, caja_recibo) VALUES 
					('".$ClienteID."', 'Credito', '".$Total."', '".$ValorRC."', '".$NewInterno."', '".$Ord_Compra."', '".$Remision."', 
					'".$Factura."', NOW(), 'Aprobado', '".$_SESSION["UserCode"]."', NOW(), '".$_SESSION["UserCode"]."', '".$VendedorID."', 
					'".$CobradorID."', '".$Caja_Interno."', '".$Caja_Recibo."')";
					...
					`observaciones`, `sub_total`, `total`, `iva`, `estado`, `peso`, `aprobado_por`, `fecha_aprobado`, 
					`beneficiario_id`) VALUES ('".$NewInterno."', '".$Remision."', '".$Fecha."', '".$Factura."', 
					'".$Ord_Compra."', '".$ClienteID."', '".$Ruta."', '".$Direccion."', '".$FormaPago."', 
					'".$Caja_Recibo."', '".$VendedorID."', '".$CobradorID."', '".$Chofer."', '".$Placa."', '".$TipoServicio."', 
					'".$TipoServicioValor."', '".$TipoDcto."', '".$TipoDctoValor."', '".$Observaciones."', 
					'".$Subtotal2."', '".$Total."', '".$Iva."', 'Aprobado', '".$Peso."', '".$_SESSION["UserCode"]."', NOW(), 
					'".$BeneficiarioID."')";
				...
				else
				{
					...
					`peso`, `aprobado_por`, `fecha_aprobado`, `beneficiario_id`) VALUES 
					('".$NewInterno."', '".$Remision."', '".$Fecha."', '".$Factura."', '".$Ord_Compra."', '".$ClienteID."', '".$Ruta."',
					'".$Direccion."', '".$FormaPago."', '".$VendedorID."', '".$CobradorID."', '".$Chofer."', '".$Placa."', '".$TipoServicio."', 
					'".$TipoServicioValor."', '".$TipoDcto."', '".$TipoDctoValor."', '".$Observaciones."', '".$Subtotal2."', '".$Total."', '".$Iva."', 'Aprobado', 
					'".$Peso."', '".$_SESSION["UserCode"]."', NOW(), '".$BeneficiarioID."')";
...
else if (isset($_GET['Ventas_Ajustar']))
{
	...
	foreach($data as $item)
	{
		...
		$query = "SELECT codigo FROM fact_movs WHERE codigo = '".$Codigo."' 
		...
		if ($final == false) {
			...
			if ($FormaPago == "Credito")
			{
				...
				if (mysql_num_rows($result) > 0)
				{
					...
					$query = "INSERT INTO mov_clientes 
					(cliente_id, tipo_movimiento, valor, saldo, interno, orden_compra, remision, factura, fecha, estado, 
					digitado_por, fecha_digitado, aprobado_por, fecha_modificado, vendedor_codigo, cobrador_codigo) VALUES 
					('".$ClienteID."', 'Debito', '".$Total."', '".$Total."', '".$Interno."', '".$Ord_Compra."', '".$Remision."', 
					'".$Factura."', '".$row["fecha"]."', 'Aprobado', '".$row["digitado_por"]."', '".$row["fecha_digitado"]."', 
					'".$row["aprobado_por"]."', '".$row["fecha_modificado"]."', '".$row["vendedor_codigo"]."', '".$row["cobrador_codigo"]."')";
...
else if (isset($_GET['CxP_AplicarPago']))
{
	...
	foreach($data as $item)
	{
		...
		WHERE compra_interno = '".$Compra_Interno."' AND tipo_movimiento = 'Compra' AND cliente_id = '".$ClienteID."'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #4: ".mysql_error():"");
...
else if (isset($_GET['Caja_Aprobar']))
{
	$Estado = isset($_GET['Estado']) ? $_GET['Estado']:"Pendiente";
	$ClienteID = isset($_GET['ClienteID']) ? $_GET['ClienteID']:0;
	$ReciboCaja = isset($_GET['ReciboCaja']) ? $_GET['ReciboCaja']:0;
	$CajaInterno = isset($_GET['CajaInterno']) ? $_GET['CajaInterno']:0;
	
	$query = "UPDATE caja_final SET estado = '".$Estado."'";
	$query .= ($Estado == "Anulado") ? ", anulado_por = '".$_SESSION["UserCode"]."', fecha_anulado = NOW()":"";
	$query .= ($Estado == "Aprobado") ? ", aprobado_por = '".$_SESSION["UserCode"]."', fecha_aprobado = NOW()":"";
	$query .= " WHERE caja_interno = '".$CajaInterno."' AND caja_recibo = '".$ReciboCaja."' AND cliente_id = '".$ClienteID."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1-1: ".mysql_error():"");
	
	$query = "UPDATE bancos SET estado = '".$Estado."' 
	WHERE caja_interno = '".$CajaInterno."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1-2: ".mysql_error():"");
	
	$query = "UPDATE cheques SET estado = '".$Estado."' 
	WHERE caja_interno = '".$CajaInterno."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1-2: ".mysql_error():"");
	
	$query = "SELECT * FROM mov_clientes WHERE caja_interno = '".$CajaInterno."' AND cliente_id = '".$ClienteID."'";
	$result1 = mysql_query($query) or die($DEBUG ? "SQL ERROR #3-1: ".mysql_error():"");
	
	$query = "SELECT * FROM cxp_movs WHERE caja_interno = '".$CajaInterno."' AND cliente_id = '".$ClienteID."'";
	$result2 = mysql_query($query) or die($DEBUG ? "SQL ERROR #4-1: ".mysql_error():"");
	
	if (mysql_num_rows($result1) > 0)
	{
		$query = "UPDATE mov_clientes SET estado = '".$Estado."'";
		$query .= ($Estado == "Anulado") ? ", anulado_por = '".$_SESSION["UserCode"]."', fecha_anulado = NOW()":"";
		$query .= ($Estado == "Aprobado") ? ", aprobado_por = '".$_SESSION["UserCode"]."', fecha_aprobado = NOW()":"";
		$query .= " WHERE caja_interno = '".$CajaInterno."' AND cliente_id = '".$ClienteID."'";
...
else if (isset($_GET['Compras_Autorizar']))
{
	...
	doc_transp = '".$Doc_Transp."', 
	estado = '".$Estado."', ";
	$query .= ($Estado == "Anulado") ? " anulado_por = '".$_SESSION["UserCode"]."', motivo_anulado = '".$Motivo_Anulado."', fecha_anulado = NOW() ":"";
	$query .= ($Estado == "Autorizado") ? " autorizado_por = '".$_SESSION["UserCode"]."', fecha_autorizado = NOW() ":"";
...
else if (isset($_GET['Compras_Ingresar']))
{
	...
	foreach($data as $item)
	{
		...
		if ($final == false)
		{
			...
			//-- CxP Movs
			$query = "INSERT INTO cxp_movs
			(`compra_interno`, `compra_entrada`, `factura`, `cliente_id`, `tipo_movimiento`, `valor`, 
			`saldo`, `estado`, `digitado_por`, `fecha_digitado`, `grupo`, `subgrupo`, `subgrupo2`) VALUES
			('".$Interno."', '".$Entrada."', '".$Factura."', '".$ClienteID."', 'Compra', '".$Total."', 
			'".$Total."', 'Aprobado', '".$_SESSION["UserCode"]."', NOW(), 'Inventarios', 'Inventarios', 'Inventarios')";
			$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #8: ".mysql_error():"");
		}
...
else if (isset($_GET['Compras_Ajustes']))
{
	...
	foreach($data as $item)
	{
		...
		if ($final == false)
		{
			...
			{
				$row = mysql_fetch_array($result);
				Anular_CxPMovs($Entrada, $Interno, "Anulado Automaticamente por Compras -> Ajuste de Ingreso");
				
				$query = "INSERT INTO cxp_movs 
				(compra_interno, compra_entrada, factura, doc_transp, cliente_id, tipo_movimiento, valor, saldo, estado, 
				digitado_por, fecha_digitado, aprobado_por, fecha_modificado, origen1, origen2, origen3, origen_documento, 
				caja_interno, caja_recibo, grupo, subgrupo, subgrupo2) VALUES 
				('".$Interno."', '".$Entrada."', '".$Factura."', '".$Doc_Transp."', '".$ClienteID."', '".$Compra."', 
				'".$Total."', '".$Total."', 'Aprobado', '".$row["digitado_por"]."', '".$row["fecha_digitado"]."', 
				'".$row["aprobado_por"]."', '".$row["fecha_modificado"]."', '".$row["origen1"]."', '".$row["origen2"]."', 
				'".$row["origen3"]."', '".$row["origen_documento"]."', '".$row["caja_interno"]."', '".$row["caja_recibo"]."', 
				'".$row["grupo"]."', '".$row["subgrupo"]."', '".$row["subgrupo2"]."')";
				$result = mysql_query($query) or die("SQL Error 7-2: " . mysql_error());
			}
	...
	foreach($data as $item)
	{
		...
		for ($i = 0; $i < $Cuotas; $i++)
		{
			...
			if ($i == 0)
			{
				...
				{
					// to 14
					$NextCuota = date("Y-n", strtotime("".$Fecha."+ ".($i+1)." month"));
			...
			'".$Interno."', '".$Nombre."', '".$Nombre."', '".$Nombre."', '".$FechaPago."', 
			'Pendiente', '".$_SESSION["UserCode"]."', NOW(), '".$Caja_Interno."', '".$Caja_Interno."')";
...
else if (isset($_GET['Nomina_Aprobar_Prestamos']))
{
	...
	if ($Estado == "Aprobado")
	{
		...
		$query = "UPDATE nom_prestamos_movs SET estado = 'Aprobado', aprobado_por = '".$_SESSION["UserCode"]."', 
	...
	if ($Estado == "Anulado")
	{
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			Anular_Caja_Recibo($row["caja_interno"], $row["caja_interno"]);
		}
	}
}
...
else if (isset($_GET["Nomina"]))
{
	...
	(fecha, caja_interno, caja_recibo, categoria, grupo, subgrupo, subgrupo2, cliente_id, efectivo, 
	total, digitado_por, fecha_digitado, estado) VALUES 
	(NOW(), '".$Caja_Interno."', '".$Caja_Interno."', 'Egresos', 'Nómina', '".$SubGrupo."', '".$SubGrupo2."', 
	'".$MAIN_ID."', '".$Total."', '".$Total."', 'SISTEMA', NOW(), 'Aprobado')";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
	...
	foreach($data as $item)
	{
		...
		AND cliente_id = '".$item["ClienteID"]."' AND tipo_movimiento = 'Debito' AND estado = 'Aprobado'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #4: ".mysql_error():"");
		if (mysql_num_rows($result) > 0)
		{
			while($row = mysql_fetch_array($result))
			{
				...
				//AND orden_compra = '".$row["orden_compra"]."'";
				//$result1 = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
...
else if (isset($_GET['Produccion_Crear']))
{
	...
	$ClienteID = (isset($main_data[0]["ClienteID"]) && $main_data[0]["ClienteID"] >= 0) ? $main_data[0]["Trefilado"]:"";
...
else if (isset($_GET['Produccion_Modificar']))
{
	...
	foreach($data1 as $item)
	{
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			origen = '".$Origen1."', 
			destino = '".$Destino1."' 
			WHERE codigo = '".$Codigo1."' AND tipo = 'Requerido' AND orden_produccion = '".$Ord_Produccion."' AND cliente_id = '".$ClienteID."'";
			$result = mysql_query($query) or die ("SQL ERROR #8-2: " . mysql_error());
		}
		else
		{
	...
	foreach($data2 as $item)
	{
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			origen = '".$Origen2."', 
			destino = '".$Destino2."' 
			WHERE codigo = '".$Codigo2."' AND tipo = 'Obtener' AND orden_produccion = '".$Ord_Produccion."' AND cliente_id = '".$ClienteID."'";
...
else if (isset($_GET['Produccion_Aprobar']))
{
	...
	else if ($Estado == "Aprobado")
	{
		...
		$query = "SELECT * FROM produccion_movs WHERE orden_produccion = '".$Ord_Produccion."' AND 
		orden_compra = '".$Ord_Compra."' AND cliente_id = '".$ClienteID."'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2-1: ".mysql_error():"");
	...
	if ($Mess == "OK")
	{
		...
		$query .= ($Estado == "Anulado") ? ", anulado_por = '".$_SESSION["UserCode"]."', fecha_anulado = NOW()":"";
		$query .= ($Estado == "Aprobado") ? ", aprobado_por = '".$_SESSION["UserCode"]."', fecha_aprobado = NOW()":"";
		$query .= " WHERE orden_produccion = '".$Ord_Produccion."' AND orden_compra = '".$Ord_Compra."' AND cliente_id = '".$ClienteID."'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
	}
	//--
	echo $Mess;
...
else if (isset($_GET['Produccion_Trefilado']))
{
	...
	// 1º
	$delete = true;
	$query = "SELECT codigo FROM produccion_proc_movs WHERE tipo = 'Obtener' 
	...
	if ($numrow > 0)
	{
		while ($rows = mysql_fetch_array($result))
		{
			...
			if ($delete == true){
				//echo "borrar!!";
				$query = "DELETE FROM produccion_proc_movs WHERE codigo = '".$rows['codigo']."' AND 
				tipo = 'Obtener' AND origen = 'Trefilado' AND orden_produccion = '".$Ord_Produccion."' ";
				$result2 = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
			}
			$delete = true;
		}
	...
	// 2º
	$delete = true;
	$query = "SELECT codigo FROM produccion_proc_movs WHERE tipo = 'Despunte' 
	AND origen = 'Trefilado' AND orden_produccion = '".$Ord_Produccion."' ";
	$result = mysql_query($query) or die ("SQL Error 1: " . mysql_error());
	$numrow = mysql_num_rows($result);
	if ($numrow > 0){
	...
	for ($i=0; $i<$num; $i++)
	{
		$Codigo1 = $data1[$i]['CodFab'];
		$Cantidad1 = $data1[$i]['Cantidad'];
		$Maquinaria = $data2[$i]['Maquinaria'];
		$Operario = $data2[$i]['Operario'];
		
		$query = "SELECT codigo FROM produccion_proc_movs WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' 
		AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Trefilado'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
		
		if (mysql_num_rows($result) > 0)
		{
			$query = "UPDATE produccion_proc_movs SET 
			cantidad = '".$Cantidad1."', 
			maquinaria = '".$Maquinaria."', 
			operario = '".$Operario."' 
			WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' AND orden_produccion = '".$Ord_Produccion."' AND 
			cliente_id = '".$ClienteID."' AND origen = 'Trefilado'";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
		else
		{
			...
			('".$Codigo1."', '".$Cantidad1."', 'Obtener', 'Trefilado', 'Trefilado', '".$Maquinaria."', 
			'".$Operario."', '".$Ord_Produccion."', '".$ClienteID."')";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
	}
	...
	if ($data3 != 0)
	{
		foreach($data3 as $item)
		{
			...
			$query = "SELECT codigo FROM produccion_proc_movs WHERE codigo = '".$Codigo2."' AND tipo = 'Despunte' 
			AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Trefilado' ";
			$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
			
			if (mysql_num_rows($result) > 0)
			{
				$query = "UPDATE produccion_proc_movs SET cantidad = '".$Cantidad2."' WHERE codigo = '".$Codigo2."' 
				AND tipo = 'Despunte' AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Trefilado' ";
			...
			else
			{
				$query = "INSERT INTO produccion_proc_movs 
				(codigo, cantidad, tipo, origen, destino, orden_produccion, cliente_id) VALUES 
				('".$Codigo2."', '".$Cantidad2."', 'Despunte', 'Trefilado', 'Trefilado', '".$Ord_Produccion."', '".$ClienteID."')";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
			
			if ($Type == "Finalizar")
			{
				AfectarInventario($Codigo2, $Cantidad2, 0, $Ord_Produccion, "Despunte", "Produccion_Producir");
			}
		}
	}
}
else if (isset($_GET['Produccion_Enderezado']))
{
	...
	if ($numrow > 0){
		while ($rows = mysql_fetch_array($result))
		{
			...
			if ($delete == true){
				//echo "borrar!!";
				$query = "DELETE FROM produccion_proc_movs WHERE codigo = '".$rows['codigo']."' AND 
				tipo = 'Obtener' AND origen = 'Corte y Enderezado' AND orden_produccion = '".$Ord_Produccion."' ";
				$result2 = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
			}
			$delete = true;
		}
	...
	for ($i=0; $i<$num; $i++)
	{
		...
		$query = "SELECT codigo FROM produccion_proc_movs WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' 
		AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Corte y Enderezado'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			operario = '".$Operario."' 
			WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' AND orden_produccion = '".$Ord_Produccion."' AND 
			cliente_id = '".$ClienteID."' AND origen = 'Corte y Enderezado'";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
		else
		{
			$query = "INSERT INTO produccion_proc_movs 
			(codigo, cantidad, tipo, origen, destino, maquinaria, operario, orden_produccion, cliente_id) VALUES 
			('".$Codigo1."', '".$Cantidad1."', 'Obtener', 'Corte y Enderezado', 'Corte y Enderezado', '".$Maquinaria."', 
			'".$Operario."', '".$Ord_Produccion."', '".$ClienteID."')";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
	}
	...
	if ($data3 != 0)
	{
		foreach($data3 as $item)
		{
			...
			{
				$query = "UPDATE produccion_proc_movs SET cantidad = '".$Cantidad2."' WHERE codigo = '".$Codigo2."' 
				AND tipo = 'Despunte' AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Corte y Enderezado' ";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
			else
			{
				$query = "INSERT INTO produccion_proc_movs 
				(codigo, cantidad, tipo, origen, destino, orden_produccion, cliente_id) VALUES 
				('".$Codigo2."', '".$Cantidad2."', 'Despunte', 'Corte y Enderezado', 'Corte y Enderezado', '".$Ord_Produccion."', '".$ClienteID."')";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
...
else if (isset($_GET['Produccion_ElectroSoldado']))
{
	...
	if ($numrow > 0){
		while ($rows = mysql_fetch_array($result))
		{
			...
			if ($delete == true){
				//echo "borrar!!";
				$query = "DELETE FROM produccion_proc_movs WHERE codigo = '".$rows['codigo']."' AND 
				tipo = 'Obtener' AND origen = 'Electrosoldado' AND orden_produccion = '".$Ord_Produccion."' ";
				$result2 = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
			}
			$delete = true;
		}
	...
	for ($i=0; $i<$num; $i++)
	{
		...
		$query = "SELECT codigo FROM produccion_proc_movs WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' 
		AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Electrosoldado'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			operario = '".$Operario."' 
			WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' AND orden_produccion = '".$Ord_Produccion."' AND 
			cliente_id = '".$ClienteID."' AND origen = 'Electrosoldado'";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
		else
		{
			$query = "INSERT INTO produccion_proc_movs 
			(codigo, cantidad, tipo, origen, destino, maquinaria, operario, orden_produccion, cliente_id) VALUES 
			('".$Codigo1."', '".$Cantidad1."', 'Obtener', 'Electrosoldado', 'Electrosoldado', '".$Maquinaria."', 
			'".$Operario."', '".$Ord_Produccion."', '".$ClienteID."')";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
	}
	...
	if ($data3 != 0)
	{
		foreach($data3 as $item)
		{
			...
			{
				$query = "UPDATE produccion_proc_movs SET cantidad = '".$Cantidad2."' WHERE codigo = '".$Codigo2."' 
				AND tipo = 'Despunte' AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Electrosoldado' ";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
			else
			{
				$query = "INSERT INTO produccion_proc_movs 
				(codigo, cantidad, tipo, origen, destino, orden_produccion, cliente_id) VALUES 
				('".$Codigo2."', '".$Cantidad2."', 'Despunte', 'Electrosoldado', 'Electrosoldado', '".$Ord_Produccion."', '".$ClienteID."')";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
...
else if (isset($_GET['Produccion_Figurado']))
{
	...
	if ($numrow > 0){
		while ($rows = mysql_fetch_array($result))
		{
			...
			if ($delete == true){
				//echo "borrar!!";
				$query = "DELETE FROM produccion_proc_movs WHERE codigo = '".$rows['codigo']."' AND 
				tipo = 'Obtener' AND origen = 'Figurado' AND orden_produccion = '".$Ord_Produccion."' ";
				$result2 = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
			}
			$delete = true;
		}
	...
	for ($i=0; $i<$num; $i++)
	{
		...
		$query = "SELECT codigo FROM produccion_proc_movs WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' 
		AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Figurado'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
		...
		if (mysql_num_rows($result) > 0)
		{
			...
			operario = '".$Operario."' 
			WHERE codigo = '".$Codigo1."' AND tipo = 'Obtener' AND orden_produccion = '".$Ord_Produccion."' AND 
			cliente_id = '".$ClienteID."' AND origen = 'Figurado'";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
		else
		{
			$query = "INSERT INTO produccion_proc_movs 
			(codigo, cantidad, tipo, origen, destino, maquinaria, operario, orden_produccion, cliente_id) VALUES 
			('".$Codigo1."', '".$Cantidad1."', 'Obtener', 'Figurado', 'Figurado', '".$Maquinaria."', 
			'".$Operario."', '".$Ord_Produccion."', '".$ClienteID."')";
			$result = mysql_query($query) or die ("SQL Error 6: " . mysql_error());
		}
	}
	...
	if ($data3 != 0)
	{
		foreach($data3 as $item)
		{
			...
			{
				$query = "UPDATE produccion_proc_movs SET cantidad = '".$Cantidad2."' WHERE codigo = '".$Codigo2."' 
				AND tipo = 'Despunte' AND orden_produccion = '".$Ord_Produccion."' AND origen = 'Figurado' ";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
			else
			{
				$query = "INSERT INTO produccion_proc_movs 
				(codigo, cantidad, tipo, origen, destino, orden_produccion, cliente_id) VALUES 
				('".$Codigo2."', '".$Cantidad2."', 'Despunte', 'Figurado', 'Figurado', '".$Ord_Produccion."', '".$ClienteID."')";
				$result = mysql_query($query) or die ("SQL Error 7: " . mysql_error());
			}
...
else if (isset($_GET['Maquinaria_Crear']))
{
	...
	causa, procedimiento, ";
	$query .= isset($data1[0]["ClienteID"]) ? "proveedor1, total1, ":"";
	$query .= isset($data2[0]["ClienteID"]) ? "proveedor2, total2, ":"";
	$query .= isset($data3[0]["ClienteID"]) ? "proveedor3, total3, ":"";
	...
	$query .= isset($data1[0]["ClienteID"]) ? "'".$data1[0]["ClienteID"]."', '".$data1[0]["Total"]."', ":"";
	$query .= isset($data2[0]["ClienteID"]) ? "'".$data2[0]["ClienteID"]."', '".$data2[0]["Total"]."', ":"";
	$query .= isset($data3[0]["ClienteID"]) ? "'".$data3[0]["ClienteID"]."', '".$data3[0]["Total"]."', ":"";
	...
	if (isset($data1[0]["ClienteID"]))
	{
		...
		'".$Ord_Reparacion."', '".$data1[0]["ClienteID"]."', 'Compra', '".$data1[0]["Total"]."', '".$data1[0]["Total"]."', 'Pendiente', 
	...
	if (isset($data2[0]["ClienteID"]))
	{
		$query = "INSERT INTO cxp_movs (compra_interno, compra_entrada, factura, cliente_id, tipo_movimiento, valor, saldo, 
		estado, digitado_por, fecha_digitado, grupo, subgrupo, subgrupo2) VALUES ('".$Ord_Reparacion."', '".$Ord_Reparacion."', 
		'".$Ord_Reparacion."', '".$data2[0]["ClienteID"]."', 'Compra', '".$data2[0]["Total"]."', '".$data2[0]["Total"]."', 'Pendiente', 
		'".$_SESSION["UserCode"]."', NOW(), '".$Clasificacion."', '".$Motivo."', '".$Tipo."')";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1-3: ".mysql_error():"");
	}
	...
	if (isset($data3[0]["ClienteID"]))
	{
		$query = "INSERT INTO cxp_movs (compra_interno, compra_entrada, factura, cliente_id, tipo_movimiento, valor, saldo, 
		estado, digitado_por, fecha_digitado, grupo, subgrupo, subgrupo2) VALUES ('".$Ord_Reparacion."', '".$Ord_Reparacion."', 
		'".$Ord_Reparacion."', '".$data3[0]["ClienteID"]."', 'Compra', '".$data3[0]["Total"]."', '".$data3[0]["Total"]."', 'Pendiente', 
		'".$_SESSION["UserCode"]."', NOW(), '".$Clasificacion."', '".$Motivo."', '".$Tipo."')";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1-4: ".mysql_error():"");
	}
...
else if (isset($_GET['Maquinaria_Modificar']))
{
	...
	$query .= isset($data1[0]["ClienteID"]) ? "proveedor1 = '".$data1[0]["ClienteID"]."', total1 = '".$data1[0]["Total"]."', ":"proveedor1 = '', total1 = '', ";
	$query .= isset($data2[0]["ClienteID"]) ? "proveedor2 = '".$data2[0]["ClienteID"]."', total2 = '".$data2[0]["Total"]."', ":"proveedor2 = '', total2 = '', ";
	$query .= isset($data3[0]["ClienteID"]) ? "proveedor3 = '".$data3[0]["ClienteID"]."', total3 = '".$data3[0]["Total"]."', ":"proveedor3 = '', total3 = '', ";
	$query .= "
	modificado_por = '".$_SESSION["UserCode"]."', 
	fecha_modificado = NOW() 
	...
	Anular_CxPMovs($Ord_Reparacion, $Ord_Reparacion, "Anulado Automaticamente por Maquinaria -> Modificar");
...
else if (isset($_GET['Maquinaria_Aprobar']))
{
	...
	if (mysql_affected_rows() > 0)
	{
		if ($Estado == "Anulado")
		{
			...
			{
				$row = mysql_fetch_array($result);
				Anular_CxPMovs($row[0], $Ord_Reparacion, "Anulado Automaticamente por Maquinaria -> Aprobar");
			}
		}
...
{
	$query = "UPDATE clientes SET estado_cuenta = '".$_GET['ActualizarEstado']."', 
	ultima_actualizacion = NOW() WHERE cliente_id = '".$_GET['ClienteID']."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
}
else if (isset($_GET['ActualizarCupo']))
{
	$query = "UPDATE clientes SET credito = '".$_GET['Credito']."', vigencia_credito = '".$_GET['Vigencia_Credito']."', 
	adicional = '".$_GET['Adicional']."', vigencia_adicional = '".$_GET['Vigencia_Adicional']."', credito_activo =  '".$_GET['Activo']."', 
	ultima_actualizacion = NOW() WHERE cliente_id = '".$_GET['ClienteID']."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
}
...
else if (isset($_GET["Presupuesto_Crear"]))
{
	...
	foreach($Items as $item)
	{
		...
		$query = "INSERT INTO presupuesto_movs (interno, item, codigo, cantidad, valor, clasificacion) 
		VALUES ('".$Interno."', '".$Item."', '".$Codigo."', '".$Cantidad."', '".$Valor."', '".$Class."')";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #5: ".mysql_error():"");
	}
...
else if (isset($_GET["Maquinas_Crear"]))
{
	$Today = date('Y-m-d');
	$data = $_GET["Maquinas_Crear"];
	...
	$query = "SELECT * FROM `apu_maq_".$Clasificacion."` WHERE codigo = '".$Codigo."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
	if (mysql_num_rows($result) > 0)
	{
		$row = mysql_fetch_array($result);
		...
		$query = "UPDATE `apu_maq_".$Clasificacion."` SET  
		codigo = '".$Codigo."', 
		nombre = '".$Nombre."', 
		grupo = '".$Grupo."', 
		subgrupo = '".$SubGrupo."', 
		...
		$query = "DELETE FROM `apu_maq_".$Clasificacion."_prov` WHERE codigo = '".$Codigo."'";
		$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
	}
	else
	{
		...
		$query = "INSERT INTO `apu_maq_".$Clasificacion."` (codigo, nombre, categoria, grupo, subgrupo, unidad, valor, uso, 
	...
	foreach($Proveedores as $item)
	{
		...
		$query = "INSERT INTO `apu_maq_".$Clasificacion."_prov` (codigo, proveedor_id, proveedor_codigo) 
...
else if (isset($_GET["Presupuesto_Clasificacion"]))
{
	...
	else
	{
		...
		$Maquinas = "apu_maq_".$Clasificacion."";
		$Maquinas_Prov = "apu_maq_".$Clasificacion."_prov";
		...
		$query = "CREATE TABLE `".$Maquinas_Prov."` ( ";
		$query .= "`id` int(11) NOT NULL AUTO_INCREMENT, ";
		$query .= "`codigo` varchar(100) NOT NULL DEFAULT '', ";
...
{
	$Chofer = isset($_GET["Chofer"]) ? $_GET["Chofer"]:die();
	$ChoferID = isset($_GET["ChoferID"]) ? $_GET["ChoferID"]:die();
	
	$query = "INSERT INTO par_fac_vehiculo (chofer, chofer_id) VALUES 
	('".$Chofer."', '".$ChoferID."') ";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
}
...
else if (isset($_GET["Parametros_Actualizar_Nomina_Retenciones"]))
{
	$ID = isset($_GET["ID"]) ? $_GET["ID"]:die();
	$VAL1 = isset($_GET["VAL1"]) ? $_GET["VAL1"]:die();
	$VAL2 = isset($_GET["VAL1"]) ? $_GET["VAL2"]:die();
