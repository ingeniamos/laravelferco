	...
	if ($last_parcial == true)
	{
		foreach($data as $item)
		{
			...
			if ($final == false)
			{
				if ($FormaPago == "Efectivo")
				{
					// REVISAR SI YA POSEE UN CREDITO (DE SER ASI, YA FUE PAGADO)
					//-- Mov_Clientes - INSERT 1
					$query = "INSERT INTO mov_clientes 
					(cliente_id, tipo_movimiento, valor, saldo, interno, orden_compra, remision, factura, fecha, estado, 
					digitado_por, fecha_digitado, aprobado_por, fecha_aprobado, vendedor_codigo, cobrador_codigo) VALUES 
					...
					while($row2 = mysql_fetch_array($result2))
					{
						if ($row2["tipo"] == "Obtener")
						{
							if ($row["fecha_aprobado"] == "0000-00-00 00:00:00")
								AfectarInventario($row2["codigo"], $row2["cantidad"], 0, $Interno, "", "Produccion", "Produccion_Producir");
							else
								AfectarInventario($row2["codigo"], $row2["cantidad"], 0, $Interno, $row["fecha_aprobado"], "Produccion", "Produccion_Producir");
						}
						else if ($row2["tipo"] == "Requerido")
						{
							if ($row["fecha_aprobado"] == "0000-00-00 00:00:00")
								AfectarInventario($row2["codigo"], $row2["cantidad"], 0, $Interno, "", "Produccion", "Produccion_Consumir");
							else
								AfectarInventario($row2["codigo"], $row2["cantidad"], 0, $Interno, $row["fecha_aprobado"], "Produccion", "Produccion_Consumir");
						}
					}
		...
		if (isset($DataDBF))
		{
			...
			for ($i = 0; $i < count($MAT_Array); $i++)
			{
				if (substr_count($MAT_Array[$i]["CODMAT"], ".") == 2)
				{
					...
					if ($Grupo == "" || strlen($Grupo) < 2)
					{
						echo "Material Perdido -> ".$MAT_Array[$i]["DESMAT"]."<br />";
						continue;
					}
					
					if ($SubGrupo == "" || strlen($SubGrupo) < 2)
					{
						echo "Material Perdido -> ".$MAT_Array[$i]["DESMAT"]."<br />";
						continue;
					}
					...
					('".$Codigo."', '".$Nombre."', 'Materiales', '".$Grupo."', '".$SubGrupo."', 
					'".$MAT_Array[$i]["UNIMAT"]."', '".$MAT_Array[$i]["VALMAT"]."', '100', 
			...
			*/
			...
			for ($i = 0; $i < count($UNI_Array); $i++)
			{
				...
				$Codigo = SUBSTR($UNI_Array[$i]["CODMAT"], 4);
				$Codigo = $Grupo.".".$SubGrupo.".".$Codigo;
				
				//checkar que grupo sea un numero o si no continuar
			...
			for ($i = 0; $i < count($BAS_Array); $i++)
			{
				...
				switch($BAS_Array[$i]["TIPOBAS"])
				{
					case "1":
						...
						if ($Objeto_Grupo == "" || strlen($Objeto_Grupo) < 2)
						{
							echo "APU_Mov -> Material Perdido -> ".$BAS_Array[$i]["CODBAS"]."<br />";
							continue;
						}
						
						if ($Objeto_SubGrupo == "" || strlen($Objeto_SubGrupo) < 2)
						{
							echo "APU_Mov -> Material Perdido -> ".$BAS_Array[$i]["CODBAS"]."<br />";
							continue;
						}
					...
					case "2":
						$Tipo = "Mano de Obra";
						$Objeto_Codigo = SUBSTR($BAS_Array[$i]["CODBAS"], 1);
						if ($Objeto_Codigo == "")
						{
