...
{
	$query = "SELECT cod_fab, nombre FROM productos WHERE categoria = 'Hierro' AND grupo = 'despunte' ORDER BY nombre ASC";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
...
else if (isset($_GET["Productos_Para_Figuracion"]))
{
	...
	WHERE categoria = 'Hierro' AND grupo = 'Figurado' OR categoria = 'Alambre' AND grupo = 'Alambre' OR categoria = 'Hierro' AND grupo = 'Malla' ORDER BY nombre ASC";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
...
else if (isset($_GET["Figuracion_CargarMalla"]))
{	
	$ProductInfo = GetProductFullInfo("Productos");
	
	$query = "SELECT * FROM mallas_movs WHERE interno_malla = '".$_GET["Figuracion_CargarMalla"]."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_array($result))
		{
			$Items[] = array(
				...
				"sepB" => $row["sepB"],
				"cod1" => $row["cod1"],
				"producto1" => isset($ProductInfo[$row["cod1"]]) ? $ProductInfo[$row["cod1"]]["Nombre"]:0,
				"cod2" => $row["cod2"],
				"producto2" => isset($ProductInfo[$row["cod2"]]) ? $ProductInfo[$row["cod2"]]["Nombre"]:0,
				"varA" => $row["varA"],
				"varB" => $row["varB"],
				"pesoA" => $row["pesoA"],
				"pesoB" => $row["pesoB"],
	...
	$query = "SELECT * FROM mallas WHERE cons = '".$_GET["Figuracion_CargarMalla"]."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
	if (mysql_num_rows($result) > 0)
	{
		$row = mysql_fetch_array($result);
		$data[] = array(
			"interno_venta" => $row["interno_venta"],
...
else if (isset($_GET["Mallas_Imprimir"]))
{
	$ClienteNombre = ClientesNombre();
	$ProductInfo = GetProductFullInfo("Productos");
	$ItemMalla = isset($_GET['itemMalla']) ? $_GET['itemMalla']:"";
	
	if($ItemMalla != ""){
		$query = "SELECT * FROM mallas_movs WHERE interno_malla = '".$_GET["Mallas_Imprimir"]."' AND iditem = '".$_GET["itemMalla"]."' ORDER BY cons ASC";
	} else {
		$query = "SELECT * FROM mallas_movs WHERE interno_malla = '".$_GET["Mallas_Imprimir"]."' ORDER BY cons ASC";
	}	
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #1: ".mysql_error():"");
	if (mysql_num_rows($result) > 0)
	{
	...
	$query = "SELECT * FROM mallas WHERE cons = '".$_GET["Mallas_Imprimir"]."'";
	$result = mysql_query($query) or die($DEBUG ? "SQL ERROR #2: ".mysql_error():"");
	if (mysql_num_rows($result) > 0)
	{
		$row = mysql_fetch_array($result);
		$data[] = array(
			"interno_venta" => $row["interno_venta"],
			"interno_malla" => $row["interno_malla"],
			"cliente_id" => $row["cliente_id"],
			"Cliente" => isset($ClienteNombre[$row["cliente_id"]]) ? $ClienteNombre[$row["cliente_id"]]:0,
			"obra" => $row["obra"],
			"pesoTotal" => $row["pesoTotal"],
			"Digitado_Por" => $row["digitado_por"],
			"Fecha_Digitado" => $row["fecha_digitado"],			
...
else if (isset($_GET['Reportes_Ventas_Mov']))
{
	...
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_array($result))
		{
			...
			$Vehiculo[$row["interno"]] = $row["placa"];
	...
	if (mysql_num_rows($result) > 0)
	{
		while ($row = mysql_fetch_array($result))
		{
			...
			$Peso = isset($Costo_Prom[$row["codigo"].$row["interno"]]) ?
			$Costo_Prom[$row["codigo"].$row["interno"]] * $Cantidad:0;
			...
			if ($OrderBy == "Cliente")
			{
				$data[] = array(
					...
					"Remision" => $row["remision"],
					"ClienteID" => $row["cliente_id"],
					"Vehiculo" => isset($Vehiculo[$row["interno"]]) ? $Vehiculo[$row["interno"]]:"",
					...
					"Peso" => isset($ProductoPeso[$row["codigo"]]) ? $ProductoPeso[$row["codigo"]]* $Cantidad:"No Existe!",
					"Cantidad" => $Cantidad,
					"Unitario" => $row["precio"],
