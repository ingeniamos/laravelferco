...
$(document).ready(function ()
{
	...
	var ClienteSource =
	{
		...
		$("#movimientos_cliente").jqxComboBox({source: ClienteDataAdapter});
	...
	$("#movimientos_cliente").bind('change', function (event) {
		...
		{
			ID_Cliente = "";
			$("#movimientos_cliente").jqxComboBox('clearSelection');
			$("#movimientos_cliente_ID").jqxComboBox('clearSelection');
		}
		else {
	...
	$("#movimientos_cliente_ID").on('select', function (event) {
		if (event.args) {
			ID_Cliente = event.args.item.value;
			clearTimeout(Timer1);
