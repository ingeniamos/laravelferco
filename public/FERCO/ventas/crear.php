...
{
	//ACA ME QUEDE..
	/*var TmpArray = new Array(<?php print_r($Listado); ?>);
	//TmpArray = JSON.parse("<?php print_r($Listado); ?>");
	...
	function LoadParameters()
	{
		var ClientesDataAdapter = new $.jqx.dataAdapter(ClientesSource,
		{
			...
			{
				$("#crear_cliente<?php echo $NumOfID ?>").jqxComboBox({source: records});
				$("#crear_cliente_ID<?php echo $NumOfID ?>").jqxComboBox({source: records});
				$("#crear_beneficiario<?php echo $NumOfID ?>").jqxComboBox({source: records});
				$("#crear_beneficiario_ID<?php echo $NumOfID ?>").jqxComboBox({source: records});
			},
		});
		...
		var ProductosDataAdapter = new $.jqx.dataAdapter(ProductosSource,
		{
			...
			loadComplete: function (records)
			{
				$("#crear_codigo<?php echo $NumOfID ?>").jqxComboBox({source: records});
				$("#crear_producto<?php echo $NumOfID ?>").jqxComboBox({source: records});
			},
		});
		
		ConductorSource.data = {"Chofer":true};
		var ConductorDataAdapter = new $.jqx.dataAdapter(ConductorSource);
		$("#crear_conductor<?php echo $NumOfID ?>").jqxComboBox({source: ConductorDataAdapter});
		
		VehiculoSource.data = {"Vehiculo":true};
		var VehiculoDataAdapter = new $.jqx.dataAdapter(VehiculoSource);
		$("#crear_placa<?php echo $NumOfID ?>").jqxComboBox({source: VehiculoDataAdapter});
	...
	function ReDefine()
	{
		...
		if ("<?php echo $Interno ?>" == "")
		{
			ClearJSON = [
				...
				{id:"crear_cupo_cr_check2<?php echo $NumOfID ?>", type:"jqxCheckBox"},
	...
	function LoadValues ()
	{
		...
		var GetValuesAdapter = new $.jqx.dataAdapter(GetValuesSource,{
			...
			loadComplete: function ()
			{
				...
				$("#crear_cliente<?php echo $NumOfID ?>").jqxComboBox("addItem", { label: records[0]["Cliente"], value: records[0]["ClienteID"]} );
				$("#crear_cliente_ID<?php echo $NumOfID ?>").jqxComboBox("addItem", { label: records[0]["ClienteID"], value: records[0]["ClienteID"]} );
			...
			?>
				...
				$("#crear_conductor<?php echo $NumOfID ?>").val(records[0]["Conductor"]);
				$("#crear_placa<?php echo $NumOfID ?>").val(records[0]["Placa"]);
	...
	function CargarValores ()
	{
		...
		var ValoresDataAdapter = new $.jqx.dataAdapter(ValoresSource,{
			...
			loadComplete: function ()
			{
				...
				$("#crear_cupo_cr_check2<?php echo $NumOfID ?>").val(records[0]["CupoAD_Check"])
	...
	$("#crear_cliente<?php echo $NumOfID ?>").on('change', function (event)
	{
		if (event.args)
		{
	...
	$("#crear_cliente_ID<?php echo $NumOfID ?>").on('change', function (event)
	{
		if (event.args)
		{
	...
	$("#crear_fecha_rem<?php echo $NumOfID ?>").jqxDateTimeInput('setDate', new Date(currenttime));
	...
	$("#crear_cupo_cr_check2<?php echo $NumOfID ?>").jqxCheckBox({
		theme: mytheme2,
		boxSize: 18,
		disabled: true
	});
	...
	function Calcular(Ignore)
	{
		var Calc_Timer = setTimeout(function()
		{
			...
			// if ($('#crear_fecha_rem<?php echo $NumOfID ?>').val() >= '11-ene-2017') {
			// 	iva = Math.round(total - (total / 1.19));
			// }
			// else {
		...
		else
		{
			...
			$.ajax({
				...
				success: function (data)
				{
					...
					document.getElementById("cancel").value = "Aceptar";
					Duplicar ? 
					Alerts_Box("Remision Duplicada con Exito!<br/>Codigo Interno Generado = \""+data[0]["Interno"]+"\"", 2)
					:
					Alerts_Box("Datos Guardados con Exito!<br/>Codigo Interno Generado = \""+data[0]["Interno"]+"\"", 2, true);
					$("#crear_interno<?php echo $NumOfID ?>").val(data[0]["Interno"]);
					$("#crear_ord_produccion<?php echo $NumOfID ?>").val(data[0]["Ord_Produccion"]);
	...
	$("#crear_ord_compra<?php echo $NumOfID ?>").jqxInput('focus');
		...
		<table cellpadding="1" cellspacing="1">
			...
			<tr>
				...
				<td colspan="3">
					...
					<li>
						<div id="crear_cupo_cr_check2<?php echo $NumOfID ?>"></div>
					</li>
				</td>
			</tr>
			<tr>
