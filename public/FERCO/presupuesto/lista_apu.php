...
$(document).ready(function ()
{
	...
	var Body = document.getElementById("Presupuesto_Lista_APU_Content");
	var Times = 0;
	var Refresh = 0;
	var Hide = 0;
	...
	if (isset($_SESSION["UserID"]) && $_SESSION["UserID"] != "")
	{
		...
		if ($data[0]["Lvl"] != "Administrador") {
			for ($i = 0; $i < $num; $i++)
			{
				if ($data[$i]["Modulo"] == "Presupuesto" && $data[$i]["SubModulo"] == "Lista_APU" && $data[$i]["Guardar"] == "true")
				{
		?>
					Guardar = true;
		<?php
				...
				if ($data[$i]["Modulo"] == "Presupuesto" && $data[$i]["SubModulo"] == "Lista_APU" && $data[$i]["Modificar"] == "true")
				{
		?>
					Modificar = true;
		<?php
	...
	function LoadParameters()
	{
		...
		$("#presupuesto_lista_apu_clasificacion").jqxComboBox({source: ClasificacionDataAdapter});
		...
		var Item = $("#presupuesto_lista_apu_clasificacion").jqxComboBox("getSelectedItem");
		if (!Item)
			return;
		$("#presupuesto_lista_apu_items_grid").jqxGrid("updatebounddata");
	};
	...
	$("#presupuesto_lista_apu_clasificacion").jqxComboBox(
	{
		theme: mytheme,
		width: 120,
		height: 20,
	...
	$("#presupuesto_lista_apu_clasificacion").on("change", function (event)
	{
		if (event.args)
		{
			...
			GridSource.data = {
				"Presupuesto_Listado_APU":true,
				"Clasificacion":Clasificacion,
			};
			var GridDataAdapter = new $.jqx.dataAdapter(GridSource);
			$("#presupuesto_lista_apu_items_grid").jqxGrid({source: GridDataAdapter});
		}
		else
		{
			Clasificacion = "";
			$("#presupuesto_lista_apu_clasificacion").jqxComboBox("clearSelection");
			$("#presupuesto_lista_apu_items_grid").jqxGrid("clearSelection");
			$("#presupuesto_lista_apu_items_grid").jqxGrid("clear");
		}
	});
	$("#presupuesto_lista_apu_clasificacion").on("bindingComplete", function (event)
	{
		if (Clasificacion != "")
			$("#presupuesto_lista_apu_clasificacion").jqxComboBox("selectItem", Clasificacion);
	});
	...
	var GridSource =
	{
		...
		updaterow: function (rowid, rowdata, commit)
		{
			$.ajax({
				...
				data: {
					"Presupuesto_Listado_APU_Actualizar":true,
					"Clasificacion":Clasificacion,
					"ID":rowdata.ID,
					"Nombre":rowdata.Nombre,
					"Unidad":rowdata.Unidad,
					"Desperdicios":rowdata.Desperdicios,
					"Gastos":rowdata.Gastos,
					"Valor":rowdata.Valor,
					"Notas":rowdata.Notas,
				},
		...
		deleterow: function (rowid, commit)
		{
			...
			var CheckTimer = setInterval(function()
			{
				if (ClickOK == true)
				{
					...
					var ID = $("#presupuesto_lista_apu_items_grid").jqxGrid("getcellvalue", rowid, "ID");
					var Codigo = $("#presupuesto_lista_apu_items_grid").jqxGrid("getcellvalue", rowid, "Codigo");
					$.ajax({
						dataType: "json",
						type: "POST",
						url: "modulos/guardar.php",
						data: {
							"Presupuesto_Listado_APU_Borrar":true,
							"Clasificacion":Clasificacion,
							"ID":ID,
							"Codigo":Codigo,
						},
						success: function (data, status, xhr)
						{
							switch(data[0]["MESSAGE"])
							{
								...
								case "USED":
									commit(false);
									Alerts_Box("No es posible borrar este elemento debido a que esta siendo usado por un presupuesto.", 4);
								break;
	...
	$("#presupuesto_lista_apu_items_grid").jqxGrid(
	{
		theme: mytheme,
		width: 1000,
		autoheight: true,
		...
		[
			{
				...
				buttonclick: function ()
				{
					if (Admin)
					{
						var selectedrowindex = $("#presupuesto_lista_apu_items_grid").jqxGrid("getselectedrowindex");
						var rowscount = $("#presupuesto_lista_apu_items_grid").jqxGrid("getdatainformation").rowscount;
						//if (selectedrowindex >= 0 && selectedrowindex < rowscount) { // Bug with RowFilter
						if (selectedrowindex >= 0) {
							var id = $("#presupuesto_lista_apu_items_grid").jqxGrid("getrowid", selectedrowindex);
							$("#presupuesto_lista_apu_items_grid").jqxGrid("deleterow", id);
						}
					}
				}
			},
	...
	$("#presupuesto_lista_apu_items_grid").jqxGrid("hidecolumn", "ID");
	$("#presupuesto_lista_apu_items_grid").jqxGrid("localizestrings", localizationobj);
...
<div id="Parte1">
	<table cellpadding="3" cellspacing="1" style="margin:5px 0px 20px 0px;">
		...
		<tr>
			...
			<td>
				<div id="presupuesto_lista_apu_clasificacion"></div>
			</td>
		</tr>
	</table>
	...
	<div id="presupuesto_lista_apu_items_grid"></div>
	<br />
</div>
